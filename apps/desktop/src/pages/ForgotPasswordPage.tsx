...
export function ForgotPasswordPage(): JSX.Element {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      setMessage('Password reset instructions were sent to your inbox.')
    } catch (err) {
      ...
      <div className="w-full max-w-md rounded-2xl border border-axew-border bg-axew-surface p-8 shadow-2xl">
        <div className="mb-8 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-axew-textMuted">Password recovery</p>
          </div>
        </div>

        <div className="mb-6">
          <h2 className="text-2xl font-semibold text-axew-text">Forgot password?</h2>
          <p className="mt-2 text-sm text-axew-textMuted">Enter your email to receive a reset link.</p>
        </div>

        <form className="space-y-4" onSubmit={handleSubmit} noValidate>
          <div>
            <label htmlFor="reset-email" className="mb-1 block text-2xs font-semibold uppercase tracking-wide text-axew-textDim">
              Email address
            </label>
          ...
          {message && (
            <div role="status" className="rounded border border-green-500/40 bg-green-500/10 px-3 py-2 text-2xs text-green-200">
              {message}
            </div>
          )}
